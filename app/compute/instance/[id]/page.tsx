...
import { type Server, Flavor, getInstance, getFlavor, AddressItem, listFlavors } from "@/lib/nova"
...
import { listImages, Image, getImage } from '@/lib/glance'
...
export function formatSecurityGroupInfo(groupName: string, securityGroups: SecurityGroup[]): React.ReactNode[] | null {
  let securityGroupInfo: React.ReactNode[] = [];

  const foundSecGroup = securityGroups.find((secGroup) => secGroup.name === groupName);
  if (!foundSecGroup) return null;

  if (foundSecGroup && foundSecGroup.security_group_rules) {
    securityGroupInfo = foundSecGroup.security_group_rules.map((rule: SecurityGroupRule) => (
      <li key={rule.id}>
        ALLOW {rule.ethertype}
        {rule.protocol && ` ${rule.protocol}`}
        {(rule.port_range_min !== null || rule.port_range_max !== null) && ` ${rule.port_range_min}-${rule.port_range_max}`}
        {` ${rule.remote_group_id ? 'from' : 'to'} ${getGroupNameFromId(rule.remote_group_id,securityGroups) ?? (rule.remote_ip_prefix || (rule.ethertype === "IPv6" ? "::/0" : "0.0.0.0/0"))}`} 
      </li>
    ));
  }
...
export function getGroupNameFromId(id: string, securityGroups: SecurityGroup[]) {
...
export default async function Instance({ params }: { params: Params }) {
  ...
  const serverVolumeKeys = server['os-extended-volumes:volumes_attached'].map((volume: { id: string }) => volume.id)
  const secGroupNames = server['security_groups'].map((secGroup: { name: string }) => secGroup.name)
  ...
  if(server.image) {
    ...
    // we need to get the image id from the volume
    if(volumes) {
      const volume = volumes.find((volume: Volume) => volume.volume_image_metadata);
    ...
    <Tabs defaultValue="overview" className="max-w-screen-xl">
      ...
      <div className="font-bold text-l mt-4  pb-6"> {server.name} </div>
        ...
        <div className="flex flex-row  ml-2 pl-2 text-xs">
          ...
          <div className="basis-3/4">{server['OS-EXT-AZ:availability_zone']}</div>
        </div>
        <div className="flex flex-row  ml-2 pl-2 text-xs">
        ...
        <div className="flex flex-row  ml-2 pl-2 text-xs">
          ...
          <div className="basis-3/4">{formattedTimeSinceDate(server.created)}</div>
        </div>
        ...
        <div className="flex flex-row  ml-2 pl-2 text-xs">
          {serverAddressKeys.map((key, index) => (
            <React.Fragment key={index}>
              ...
              <div className="basis-3/4">{server.addresses[key].map((address: AddressItem) => address.addr).join(', ')}</div>
            </React.Fragment>
          ))}
        </div>
        ...
        {secGroupNames.map((secGroupName, index) => (
          <div className="flex flex-row  ml-2 pl-2 text-xs" key={index}>
            ...
            <div className="basis-1/4 mb-2 pb-2"><p className='font-bold text-xs'>{secGroupName}</p>
         </div>
          <div className='basis-3/4 mb-2 pb-2'>
            <ol>
              {
                 formatSecurityGroupInfo(secGroupName, securityGroups)
              }
            </ol>
          </div>
        </div>
                ...
                volumes.map((key, index) => (
                  <React.Fragment key={index}>
                      <div className="flex flex-row ml-2 pl-2 text-xs">
                          ...
                          <div className="basis-3/4">{key.name} on {key.attachments[0].device} </div>

                      </div>
                  </React.Fragment>
              ))
            ) : (
            <p className="ml-2 pl-2 text-xs text-gray-500">No Volumes Attached</p>
            )}

        <div className="flex flex-row  ml-2 mb-2 pl-1 pb-2 text-xs">
